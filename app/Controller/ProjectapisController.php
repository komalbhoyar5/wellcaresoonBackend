...
class ProjectapisController extends AppController {
	...
	public function beforeFilter() {
		...
		$this->Auth->allow('getProjectDetails','getCategories','getsubCategoryDetails','getBrands','getSliderImages','getCategoryProductList',
							...
							'getStatearray','getSidebarAdsForPages','place_order','updateProfile','getCustomerProfile','change_password','company_address_details');
	...
	// ================================================ General setting data ====================================================== //
		...
		public function sendEnquiryDetail(){
			...
			if (!empty($postData)) {
				...
				$return = $Component->sendContactUsmailtoAdmin($postData,$company_details['Setting'], $contact_email_id['Setting']['value']);
				if ($return == true) {
					$status = 'Success';
		...
		public function company_address_details(){
			...
			$this->loadModel('Setting');
			$details = $this->Setting->find('all', array('conditions'=>array('form_type'=>'company_address_details')));
			// print_r($details);
			$Component = $this->Components->load('General');
			// country name
			foreach ($details as $detail) {
				if ($detail['Setting']['name'] == 'country') {
					$country_id = $detail['Setting']['value'];
					$return = $Component->getCountryname($detail['Setting']['value']);
					$detail['Setting']['value'] = $return;
				}

				if ($detail['Setting']['name'] == 'state') {
					$return = $Component->getStatename($detail['Setting']['value'], $country_id);
					$detail['Setting']['value'] = $return;
				}
			...
			if (!empty($postData)) {
				...
				if ($this->User->validates()) {
					...
					if ($this->User->save($postData)) {
						...
						$status = 'Success';
						$Component = $this->Components->load('Email');
						$return = $Component->sendMail($response,$random,$company_details['Setting'], $contact_email_id['Setting']['value']);
					}else { 
						$status = 'Fail';
